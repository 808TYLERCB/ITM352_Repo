...
app.post("/login", function (request, response) {
    ...
    if (typeof user_reg_data[username_entered] != 'undefined') {
        ...
        if (password_entered == user_reg_data[username_entered].password) {
            ...
            // Determine last login message
            let lastLoginMsg = request.session.last_login ? `Your last login was: ${request.session.last_login}` : "This is your first visit!";
            // Update the last login time in the session
            request.session.last_login = new Date().toLocaleString();

            // Set a cookie with the username
            response.cookie('username', username_entered, { maxAge: 300000 }); // Expires in 5 minutes

            // Create the response message
            response_msg = `${username_entered} is logged in. ${lastLoginMsg}`;
        } else {
...
app.get('/set_cookie', function(request, response) {
    response.cookie('name', 'YourName').send('Cookie is set');
});
...
app.get('/set_cookie', function(request, response) {
    response.cookie('name', 'YourName', { maxAge: 5000 }).send('Cookie is set');
});

app.get('/use_session', function (request, response) {
    // This will print out the current session ID before destroying the session
    response.write(`Welcome, your session ID is ${request.session.id}\n`);

    // Destroy the session and end the response
    request.session.destroy(function(err) {
        if (err) {
            console.error("Error destroying session: ", err);
            response.end("Error destroying session.");
        } else {
